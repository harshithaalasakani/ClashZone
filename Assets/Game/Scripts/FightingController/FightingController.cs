...
public class FightingController : MonoBehaviour
{
    ...
    void PerformAttack(int attackIndex)
    {
        if (Time.time - lastAttackTime > attackCooldown)
        {
            ...
            foreach (Transform opponent in opponents)
            {
                ...
                {
                    opponent.GetComponent<OpponentAI>().StartCoroutine(opponent.GetComponent<OpponentAI>().PlayerDamageAnimation(attackDamages));
                }
            }
        }
        else
    ...
    public IEnumerator PlayerDamageAnimation(int takeDamage)
    {
        yield return new WaitForSeconds(0.5f);

        if (hitSounds != null && hitSounds.Length > 0)
        {
            int randomIndex = Random.Range(0, hitSounds.Length); // Fixed
